  ...
  return (
    ...
    <div className="relative break-inside-avoid group">
      <button onClick={() => agregarFavoritos({ id, title, url })} >
        <svg
          className={`z-10 w-8 h-8 absolute  group-hover:block  top-6 right-2 ${isFavorito ? 'fill-red-600 block' : 'hidden fill-white hover:fill-red-500'}`}
      ...
      <Link to={`elemento/${id}`} state={{ id, title, url }} key={id} className="mb-4 flex flex-col   ">

        <figure className="flex flex-col items-center justify-center w-full h-full relative ">
